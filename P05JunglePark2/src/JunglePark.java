...
/**
 * Class that creates and runs a JunglePark window, extends PApplet
 * 
...
public class JunglePark extends PApplet {
  ...
  public void setup() {
    ...
    //creates tiger, deer, and clear buttons and adds them to listGUI
  ...
  public void draw() {
    ...
    for (int i = 0; i < listGUI.size(); i++) {
      listGUI.get(i).draw();
      if (listGUI.get(i).getClass() == Tiger.class) {
        Tiger tempTiger = (Tiger) listGUI.get(i);
        tempTiger.action();
      } else if (listGUI.get(i).getClass() == Deer.class) {
        Deer tempDeer = (Deer) listGUI.get(i);
        tempDeer.action();
      }
    }
  }
  ...
  /**
   * This main method starts the application using PApplet
   * 
   * @param args input from user through command line
   * @see PApplet
   */
