 ...
 * 
 * @author mouna & Michael Goldstein
 *
 */
...
public class Sequence implements Iterable<Integer> {
  ...
  public Sequence(int[] command) {
    ...
    switch (sequenceType) {
      case ARITHMETIC: // call the constructor of the ArithmeticSequenceGenerator class to create
        ...
        break;
      case GEOMETRIC:
        // call the constructor of the GeometricSequenceGenerator class to create
        // an iterator over a geometric sequence with initial element command[1], ratio
        // command[2], and size stored at command[3]
        sequenceIterator = new GeometricSequenceGenerator(command[1], command[2], command[3]);
        break;
      case FIBONACCI:
        // call the constructor of the FibonacciSequenceGenerator class to create
        // an iterator over a Fibonacci sequence with initial element 1, size command[1]
        sequenceIterator = new FibonacciSequenceGenerator(command[1]);
        break;
      case DIGIT_PRODUCT:
        // call the constructor and getIterator() of the DigitProductSequenceGenerator class to
        // create an iterator over a digit product sequence with initial element command[1],
        // size command[2]
  ...
  /**
   * Is an accessor method for sequenceIterator field. Returns sequenceIterator
   * 
   * @return sequenceIterator, iterator over the created sequence
   */
  @Override
